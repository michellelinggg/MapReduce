...
public class Proj1{
    ...
    public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePair> {
        ...
        public void map(WritableComparable docID, Text docContents, Context context)
        	...
        	ArrayList<Double> targetIndexes = new ArrayList<Double>();
        	double index = 1;
        	while (matcher.find()){
        	...
        	while (matcher.find()){
        		...
        		if (!word.equals(targetGram)){
        			...
        			for (double i: targetIndexes){
        				double temp = Math.abs(index - i);
        				if (temp < distance)
        					distance = temp;
        			}
        			context.write(currentWord, new DoublePair(1d, func.f(distance)));
        		}
        		index++;
        	}
        }
        ...
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
                    Context context) throws IOException, InterruptedException {
        				double wordCount = 0d;
        				double coOccurence = 0d;
        				for (DoublePair value : values) {
        					wordCount += value.getDouble1();
        					coOccurence += value.getDouble2();
        				}
        				context.write(key, new DoublePair(wordCount, coOccurence));
            	}
    }
        ...
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
	 			...
	 			if (coOccurence == 0)
	 				...
	 				rate = (coOccurence * Math.pow((Math.log(coOccurence)), 3)) / wordCount;
	 			}
	 			context.write(new DoubleWritable(rate*-1), key);
            }
    }
    ...
    public static class Reduce2 extends Reducer<DoubleWritable, Text, DoubleWritable, Text> {
        ...
        public void reduce(DoubleWritable key, Iterable<Text> values,
        		...
        		for (Text value: values){
        			if (N_TO_OUTPUT > 0){
        				double num = key.get() * -1;
        				context.write(new DoubleWritable(num), value);
        				N_TO_OUTPUT = N_TO_OUTPUT - 1;
        		}
        	}
        }
    }
